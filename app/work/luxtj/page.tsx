...
export default function LuxtjPage(){return <main id="top" className="case-page">
  ...
  <section className="case-section case-narrow reveal"><CaseKicker>Executive summary</CaseKicker><h2>I redesigned the journey, not only the interface.</h2><p className="case-lead">The central design move was to treat a trip as one persistent object. Search, comparison, traveller details, review and partner fulfilment could then share the same context instead of behaving like disconnected screens.</p><div className="case-three"><article><span>Problem</span><h3>Complex decisions were fragmented.</h3><p>Travellers had to compare multiple products while remembering dates, people, prices and policies.</p></article><article><span>Response</span><h3>One connected trip model.</h3><p>Persistent context and reusable patterns made each step answer the next important question.</p></article><article><span>Result</span><h3>A clearer system for users and delivery.</h3><p>The work aligned customer-facing journeys and partner operations around a shared interaction language.</p></article></div></section>

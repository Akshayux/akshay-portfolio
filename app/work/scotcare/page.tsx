...
export default function ScotCarePage(){return <main id="top" className="case-page scotcare-case">
  <SiteHeader/>
  <aside className="case-toc" aria-label="Case study contents"><span>On this page</span><a href="#overview">Overview</a><a href="#problem">Problem</a><a href="#research">Research</a><a href="#experience">Experience</a><a href="#reflection">Reflection</a></aside>
  ...
  <section className="case-hero-media scotcare-hero-media reveal" data-parallax><img src="/scotcare/overview.png" alt="ScotCare connected care-navigation experience"/></section>

  <section className="case-section case-narrow reveal"><Kicker>Project position</Kicker><h2>Care navigation, not diagnosis.</h2><p className="case-lead">ScotCare helps a patient describe a concern, recognise when urgent help may be required, understand a suggested care route and stay supported while waiting, preparing and following up.</p><div className="case-three">{priorities.map(([title,copy])=><article key={title}><span>Design priority</span><h3>{title}</h3><p>{copy}</p></article>)}</div><p className="case-disclaimer"><strong>Important:</strong> ScotCare is an independent academic concept. It is not an official NHS Scotland service, does not diagnose or prescribe, and has not been clinically validated.</p></section>
